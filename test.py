...
# 페이지 설정
st.set_page_config(
    page_title="🚨 쫄깃쩔깃돌멩이님의 스마트 응급 처치 가이드",
    layout="wide",
...
first_aid_data = {
    ...
    "알레르기 반응 (아나필락시스)": {
        ...
        3.  **환자 자세**: 의식이 있다면 다리를 높여주고 눕히거나, 호흡 곤란이 있다면 편하게 앉히는 것이 좋습니다. 구토 시 고개를 옆로 돌려 기도를 확보합니다.
...
st.markdown("---") # 웹사이트 메인 타이틀 아래 구분선 유지

# --- 내용 표시 로직 ---
# '특정 증상/사고 검색' 기능이 제거되었으므로, 해당 검색 결과 표시 로직은 제거됩니다.
...
if selected_type == "소개":
    ...
    # 소개 페이지 아래의 구분선은 제거하지 않습니다. 필요에 따라 조절해주세요.
    st.markdown("---") 

else: # 특정 유형이 선택되었을 때
    info = first_aid_data[selected_type]
    with st.expander(f"{info['emoji']} **{info['제목']}**", expanded=True):
        st.subheader("📚 상황 설명")
        st.info(info['설명'])
        st.markdown("---") # 이 줄은 상황 설명과 응급 처치법 사이의 구분선입니다. 이건 유지하는 것이 가독성에 좋습니다.
        st.subheader("💡 응급 처치법")
        st.markdown(info['처치법'])
    # --- 중요 변경 사항 ---
    # 여기서 각 항목의 expander 아래에 있던 가로줄을 제거합니다.
    # st.markdown("---") # 이 줄을 제거했습니다!
