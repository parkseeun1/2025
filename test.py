...
first_aid_data = {
    "찰과상": {
        ...
        3.  **소독 및 연고 바르기🧴**: 소독약을 바르거나 항생제 연고를 얇게 바릅니다. 알레르기 반응에 주의하세요.
    ...
    "과호흡": {
        ...
        """,
        "키워드": ["숨", "답답함", "어지럼증", "손발 저림", "불안", "스트레스", "과호흡", "숨차", "가슴 아픔", "떨림"]
    },
    "근육 경련": {
        "emoji": "🦵",
        "제목": "근육 경련 (쥐남) 응급 처치",
...
# 검색 기능
search_query = st.sidebar.text_input("🔍 특정 증상/사고 검색", placeholder="예: 화상, 골절, CPR, 쥐, 냉방병", help="궁금한 응급 처치 관련 키워드를 입력해보세요.")
...
selected_type = selected_option.split(' ', 1)[1] if " " in selected_option and selected_option != "소개" else selected_option
...
if search_query:
    ...
    for key, value in first_aid_data.items():
        if search_query_lower in key.lower() or any(search_query_lower in k.lower() for k in value["키워드"]):
            with st.expander(f"{value['emoji']} **{value['제목']}**", expanded=True): # 검색 결과는 기본적으로 펼쳐서 보여줍니다.
                st.subheader("📚 상황 설명")
                st.info(value['설명']) # 설명 부분을 info 박스로 강조
                st.markdown("---")
                st.subheader("💡 응급 처치법")
    ...
    if not found_results:
        ...
        st.info("다른 키워드 (예: '열', '상처', '찰과상', '다리 쥐', '숨 가쁨')로 검색하거나, 좌측에서 주요 상처 유형을 선택해보세요.")
    ...
    with col1:
        st.markdown("### 📚 이용 방법")
        st.markdown("""
        1.  **🌟 주요 상처 유형 선택**: 좌측 사이드바에서 이모티콘과 함께 제시된 주요 사고 유형을 클릭하여 해당 처치법을 바로 확인하세요.
        2.  **🔍 키워드 검색**: '특정 증상/사고 검색' 칸에 궁금한 응급 처치와 관련된 단어(예: '불', '뼈', 'CPR', '코피', '냉방병')를 입력하여 빠르게 정보를 찾을 수 있습니다.
        """)
